    ...
    nitGenerateCardToken: function (merchant_session_key, isOSC, isFireC, suite) {
        ...
        if (merchant_session_key) {
            try {
                ...
                alert("Unable to initialize SagePay payment method, please refresh the page and try again.");
                if (isOSC) {
                    var isPiSelected = document.getElementById('p_method_sagepaynit').checked;
                    if (isPiSelected) {
                        this.resetOscLoading();
                    } else {
                        document.getElementsByClassName("onestepcheckout-place-order-loading")[0].innerHTML = old_message_html;
        ...
        new Ajax.Request(
            ...
            onSuccess: function (data) {
                ...
                try {
                    ...
                    console.log(err);
                    //checkout.setLoadWaiting(false);
                    var isPiSelected = false;
                    if ($('p_method_sagepaynit')) {
                        isPiSelected = document.getElementById('p_method_sagepaynit').checked;
                    }

                    if (isPiSelected) {
                        alert("Unable to initialize SagePay payment method, please make sure the credit card details were correctly submitted or refresh the page to try again.")
                    } else {
                        alert("Unable to initialize SagePay payment method, please refresh the page and try again.");
                    }

                    if ($("onestepcheckout-place-order-loading")) {
                        document.getElementsByClassName("onestepcheckout-place-order-loading")[0].innerHTML = old_review_message;
                    }
                }
            }
